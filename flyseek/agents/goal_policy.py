"""
Exploration goal policy (engineered, disclosed; its weights are what Phase 4 training
learns). It picks a WORLD direction that is fed to the connectome as an FC2 goal bump;
the central-complex circuit (EPG/FC2 -> PFL3 -> DNs) does the steering.

Every `decide_s` seconds (or when the current goal direction is blocked) each fly
scores K candidate directions:
  free    open distance along the direction (ray cast), normalized
  novel   how unvisited the ground along that direction is (per-fly visit map)
  align   cos(direction - current heading)  (prefer not turning around)
  keep    cos(direction - previous goal)    (prefer consistent goals)
score = w_free*free + w_novel*novel + w_align*align + w_keep*keep; the best wins.
