"""
Phase 1 regime sweep: find a calibration where the model
  (a) does NOT self-sustain after a brief pulse (no ignition), and
  (b) still carries sensory input to descending neurons.

Sweeps connectome_weight_scale x modulatory_sign. One exploratory seed per
config; the chosen config then gets full statistics in sanity_checks.py.

Batch columns (1.5 s run; the last 1.0 s is measured):
  0 none
  1 pulse_dna02   DNa02-L 100 Hz for 50 ms, then off
  2 pulse_lc10a   LC10a-L 50 Hz for 50 ms, then off
  3 lc10a_L       LC10a-L 50 Hz sustained
  4 lc10a_R       LC10a-R 50 Hz sustained
  5 loom_L        LC4+LPLC2-L 50 Hz sustained
  6 broad         all photoreceptors + ORNs 20 Hz sustained
"""
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="phase1_regime_sweep.json")
    args = ap.parse_args()
