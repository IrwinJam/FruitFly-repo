"""
Phase 4.2 go/no-go: does the central-complex pathway turn a heading bump (EPG) and
a goal bump (FC2) into a left/right steering signal that depends on goal - heading?

Stimulus (optogenetic-style Poisson drive, von Mises bumps over anatomical phase):
  EPG neuron with nominal phase phi:  rate = r_max * exp(kappa * (cos(phi - heading) - 1))
  FC2 neuron with nominal phase psi:  rate = r_max * exp(kappa * (cos(psi - goal) - 1))
  (phases from flyseek/brain/cx_map.py; nominal convention, so a constant offset is
   expected and is MEASURED, not assumed)

Conditions (one batch column each, R replicates):
  heading h in 8 directions x offset d = goal - h in 8 directions
  controls: EPG bump only (8 headings), FC2 bump only (8 goals), no input

Readouts over 1 s: PFL3 grouped by the side they steer (measured from their output
synapses), DNa02 L/R, DNa03 L/R.

Analysis per readout (replicate means):
  fit y = c + A*cos(x - phi0) with x = d (relative), x = g (goal only), x = h (heading only)
  -> R^2 for each. The circuit computes relative angle if R^2(d) >> R^2(g), R^2(h).
Real wiring vs shuffled wiring: empirical p on amplitude A and R^2(d).
"""
...
from flyseek.brain.roles import _to_graph, type_idx
...
def main():
    ...
    ap.add_argument("--out", default="phase4_cx_test")
    args = ap.parse_args()
