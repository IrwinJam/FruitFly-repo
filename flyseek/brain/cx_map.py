"""
Phase 4.1: map the central-complex compass circuit onto MaleCNS neurons.

For EPG, FC2 (A/B/C) and PFL3 neurons, parse anatomical position from `instance`:
  EPG(PB08)_R2          -> protocerebral bridge (PB) glomerulus side R, index 2
  FC2A_C2_L             -> fan-shaped body (FB) column 2
  PFL3(PB12c)_R2_C1     -> PB glomerulus R2 and FB column 1
Assigns a NOMINAL phase (PB glomerulus k -> (k-1)*45 deg; FB column c -> (c-1)*40 deg).
The nominal convention is an assumption; Phase 4.2 measures the actual heading-vs-goal
tuning, so a constant offset in this convention shows up as a measured number.

Also measures from the connectome (not from literature memory):
  - which side each PFL3 steers: its synapse weight onto left vs right DNa02/DNa03 and
    onto left vs right LAL neurons
  - total EPG->PFL3, FC2->PFL3, PFL3->DN synapse counts (direct and 2-hop via LAL)
Writes CACHE_DIR/cx_map.json and docs/phase4_cx_map.json.
"""
...
from flyseek.brain.roles import neurons
...
def main():
    ...
    ann = pd.read_feather(__import__("flyseek.paths", fromlist=["RAW_DIR"]).RAW_DIR /
    ...
    (DOCS_DIR / "phase4_cx_map.json").write_text(json.dumps(summary, indent=2, default=str))
