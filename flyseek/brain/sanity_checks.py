"""
Phase 1 sanity checks, redone with controls and statistics (replaces the M3 version,
whose flaws are described in GAMEPLAN.md section 1.1).

Uses the calibrated model in config/brain.yaml. For each graph (real and shuffled),
every condition runs as R independent replicate batch columns (independent Poisson
input draws), for a 200 ms warmup then a 1 s measurement window.

Experiments:
  pursuit   drive LC10a (L or R) at several rates -> DNa02 and DNa03 L-R rate difference
  looming   drive LC4+LPLC2 (L or R) at several rates -> DNp01 L/R rates
  ignition  50 ms pulse on one DNa02-L neuron or on LC10a-L, then silence -> any
            activity left after 500 ms?
  none      no input (baseline)
Also recorded per condition: forward/backward/arousal candidates (DNg100, DNp09,
MDN, pIP1) and measured stimulated-population rates (not just nominal rates).

Statistics per pursuit/looming contrast: mean and 95% CI across replicates,
Welch's t-test between left-drive and right-drive, and Cohen's d.
"""
...
from flyseek.brain.roles import role_idx, type_idx
...
def write_markdown(all_results: list[dict], path):
    lines = ["# Phase 1 sanity checks (calibrated model)", "",
             "Generated by `flyseek/brain/sanity_checks.py`. Values: mean [95% CI] across replicates. "
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="phase1_sanity")
    args = ap.parse_args()
