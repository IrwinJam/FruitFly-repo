...
Matches the Shiu et al. 2024 reference model (philshiu/Drosophila_brain_model):
...
* One weight matrix is shared by the whole batch; state (v, g, refractory, delayed
  currents) has a batch dimension, so a batch column can be a different fly, trial
  seed, or experimental condition.
* Integration is exact for the linear (v, g) system over one step, so dt=0.5 ms
  closely tracks the 0.1 ms Brian2 default. flyseek/brain/dt_check.py verifies this.
* Synaptic delay is a ring buffer of D = round(t_dly/dt) steps of pending current.
* Refractory period is an integer step counter: floor(t_rfc/dt) steps without
  integration after a spike (voltage held at reset), matching Brian2's
  `(t - lastspike) > refractory` test.
* Silencing (Shiu's method) zeroes a neuron's outgoing synapses; here it masks the
  neuron's spikes before propagation, which is equivalent.
* Spike propagation is either sparse matmul over the whole graph ("spmm") or
  event-driven over only the out-edges of neurons that spiked ("event"). "auto"
  picks whichever is cheaper each step.
"""
...
from flyseek.paths import CACHE_DIR, CONFIG_DIR
