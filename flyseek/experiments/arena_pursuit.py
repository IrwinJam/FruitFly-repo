"""
Phase 2 closed-loop test: does a connectome-driven fly turn toward a target it sees?

Setup: empty 10x10 arena. Each fly starts at the origin facing +x, with its own static
target 4 units away at +60 deg (left) or -60 deg (right). Flies walk at the
engineered base speed; ALL turning comes from the brain via the motor decoder
(config/motors.yaml). 20 left + 20 right trials per condition, independent Poisson
input per fly.

Conditions:
  real      navcore
  blind     navcore with the target channel switched off (no LC10a drive)
  shuffled  navcore_shuf0..K-1 (same adapter, wiring permuted)

Metrics per fly:
  correct_turn_1s:  heading change after 1 s has the target's sign and |change| > 5 deg
  bearing_reduced:  |bearing to target| at the end < at the start
  min_dist:         closest approach to the target
Pre-registered success criterion (GAMEPLAN Phase 2): real correct_turn_1s >= 70%,
and clearly above blind and shuffled.
"""
...
def main():
    ...
    (DOCS_DIR / "phase2_arena_pursuit.json").write_text(json.dumps({"summary": summary, "conditions": results}, indent=2))
