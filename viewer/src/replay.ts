...
export class Replay {
  ...
  /** Alive at this tick. A catch logged after the last recorded frame already counts on that frame. */
  alive(tick: number, agent: number): boolean {
    if (this.field(tick, agent, "alive") <= 0.5) return false;
    if (tick < this.meta.n_ticks - 1) return true;
    return !this.meta.events.some((e) => e.kind === "kill" && e.victim === agent);
  }
