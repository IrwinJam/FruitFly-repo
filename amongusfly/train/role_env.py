"""
Batched Hide n Seek episodes for role training and evaluation.

M matches run in lock-step, each with one seeker and H hiders under the rules engine
(amongusfly/world/rules.py). The trained role is played by one controller type; the other role is
scripted, unless role="both", where every fly is a brain.

  role="seeker": agent 0 is the trainee; batch column = match
  role="hider":  agents 1..H are trainees; batch column = m*H + (h-1)
  role="both":   every agent is a brain; batch column = m*(1+H) + agent

Controllers: "brain" (connectome + role policy + decoder), "scripted", "random" (random walk).

Fitness:
  seeker: sum over hiders of caught * (1 + time left / round) / H        (0..2)
  hider:  survival fraction + 0.5 if it survived the round
"""
...
from amongusfly.agents.scripted import ScriptedHider, ScriptedSeeker
...
from amongusfly.train.adapter import apply_decoder
from amongusfly.world.grid import OccupancyGrid
from amongusfly.world.match import DANGER_MAX_HZ, PING_DECAY_S, PING_MAX_HZ, room_grid, spawn_positions
...
def run_role_episode(tag: str, role: str, values: dict | None, seeds: list[int], controller: str = "brain",
                     ...
                     silence: list[str] | None = None, brain=None, record_positions: bool = False,
                     decoder_overrides: dict | None = None) -> dict:
    """values: decoded adapter values (scalars or arrays of length = number of brain flies).
    decoder_overrides: motor-config values set after the adapter, e.g. {"goal_behind.enabled": False}."""
    W = _world()
    ...
    if controller == "brain":
        ...
        for path, value in (decoder_overrides or {}).items():
            node = dcfg
    ...
    for tick in range(max_ticks):
        ...
        if pop is not None:
            ...
            goal, sp = policy.step(pop.body.x, pop.body.y, pop.body.heading, dt, movable & sensing, seen_h, pings,
                                   seen_s, danger[cm, ca], blocked=pop.body.wall_contact)
        ...
        if traj is not None:
            wc = np.zeros((M, n))  # wall contact of the brain flies, for the stall audit
            if pop is not None:
                wc[cm, ca] = pop.body.wall_contact
            traj.append(np.stack([X.copy(), Y.copy(), wc]))
