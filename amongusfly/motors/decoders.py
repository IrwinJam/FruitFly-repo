"""
Motor decoder: smoothed descending-neuron rates -> forward speed and turn rate per fly
(settings in config/motors.yaml). Any value may be a per-fly array, so a whole population of
settings runs in one batch.
"""
...
class MotorDecoder:
    ...
    def decode(self, weights: dict | None = None, base_speed: np.ndarray | float | None = None) -> DecodedCommand:
        ...
        # goal behind: PFL3 steering fades to zero when the goal is behind the
