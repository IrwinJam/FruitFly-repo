"""
Resolve AMongus Fly's sensory and motor roles to MaleCNS cell types.

Strict: every role must resolve to at least one type present in the annotations, or loading
fails. Non-obvious mappings in MaleCNS v1.0:
  - P1 is annotated as `pIP1` (synonyms pIP1 / pIP-a); `pIP10` is a separate descending neuron.
  - oDN1 is a female neuron; DNg100 (with DNp09) is used for forward drive instead.
  - R1-R6 are pooled into one type; R7 and R8 are split into d/p/y subtypes.
  - Most photoreceptors and ORNs have no soma position (their cell bodies are outside the brain).
  - Odour channels use antennal ORNs: DA2 and V (aversive) for danger, VA2 and DM1 (attractive)
    for pings.
"""
...
from amongusfly.paths import RAW_DIR as DATA_DIR
...
def load_annotations(path: Path = ANNOTATIONS_FILE) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            ...
            "`python -m amongusfly.connectome.download` first."
        )
...
def resolve_roles(df: pd.DataFrame | None = None) -> dict[str, ResolvedRole]:
    ...
    if missing:
        raise ValueError(
            ...
            "This dataset's naming may have changed; update amongusfly/connectome/celltypes.py."
        )
