"""
Replay recorder: per-tick agent states plus sparse per-fly spike counts, saved as one .npz
(arrays) and one .json (metadata).

Spikes are stored per (tick, fly) as the local indices of neurons that spiked in that tick
(uint32) and their counts (uint8, clipped at 255); offsets[t * n_flies + f] marks each block.
"""
