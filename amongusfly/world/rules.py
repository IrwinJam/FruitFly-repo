...
Phases: hide (seeker frozen) -> seek -> final_hide (seeker speed boost, periodic pings revealing
living hiders) -> over. The seeker wins when every hider is caught; hiders win if any survive.

Brains cannot press "vent", so a hider vents automatically when it is on a vent with uses left
and the seeker is within `auto_vent_danger_range`. It comes out of the linked vent farthest from
the seeker after `max_time_inside_s`, and cannot be caught or seen while inside.
"""
