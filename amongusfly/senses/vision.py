"""
Vision encoder: world -> Poisson rates for the visual neurons, per fly and per side.

  target -> LC10a: grows with the target's angular size, capped at 25 Hz.
  loom   -> LC4 + LPLC2: grows with the threat's angular expansion speed, capped at 100 Hz.
  photo  -> photoreceptors: low-rate brightness per hemifield from wall distance.

An object to the fly's left drives its left-side neurons; objects near the midline drive both.
Coding is side-level, not retinotopic.
"""
