"""
Closed-loop escape test: does an approaching threat trigger the giant-fibre escape dash?

A fly walks in an empty arena; a threat starts 5 units away on its left or right and moves
straight at it. The vision encoder turns the threat's angular expansion into LC4/LPLC2 drive,
and a dash happens when smoothed DNp01 passes the decoder threshold. Conditions: real graph,
blind (looming channel off) and shuffled graphs.
"""
...
def main():
    ...
    (DOCS_DIR / "results" / "escape_test.json").write_text(json.dumps({"summary": summary, "conditions": results}, indent=2))
