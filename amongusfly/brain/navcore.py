"""
Build navcore: the subgraph between the sensory roles and the motor descending neurons.

A neuron is kept if it lies within a few hops downstream of a sensory neuron and upstream of a
motor descending neuron, following strong edges only (an edge counts if it carries at least
`min_frac` of the neuron's input or output synapses), plus every role neuron and the compass
circuit. Indices are remapped to 0..M-1 and the mapping is saved so roles can be looked up.
"""
...
# The compass circuit: EPG (heading) and FC2 (goal) are sources, and their path to the steering
# descending neurons is always kept: Delta7, PFL3 and PFL3's strongest postsynaptic partner types
# (their per-edge input share is too small to pass min_frac).
