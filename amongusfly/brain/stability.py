"""
Stability under sustained input: drive photoreceptors and ORNs at several rates for several
seconds and check that activity settles rather than running away. Reports population rate over
time, the fraction of neurons ever active and per-superclass rates.
"""
...
from amongusfly.brain.roles import full_idx_of, neurons, role_idx
...
if __name__ == "__main__":
    ...
    for r in all_runs:
        for x in r["results"]:
            x["verdict"] = verdict(x)
    (DOCS_DIR / "results" / "brain_stability.json").write_text(json.dumps(all_runs, indent=2))
    plot(all_runs, DOCS_DIR / "results" / "brain_stability.png")
    print("saved docs/results/brain_stability.json and .png")
